...
if (!isset($userData['taskId']) || !isset($userData['newTaskName']) || !isset($userData['userId'])) {
    respond(false, 0, 'Invalid input');
}

try {
    $db = new TasksManagementsDB($host, $dbname, $username, $password);
    $task = new Task($db, $userData["newTaskName"], $userData['taskId']);
    $result = $task->updateTaskName($userData['userId']);
